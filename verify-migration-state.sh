...
COLUMNS_CHECK=$(docker compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "
  SELECT column_name
...
INDEXES_CHECK=$(docker compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "
  SELECT indexname
  FROM pg_indexes
...
if [ "$COLUMN_COUNT" -eq 5 ] && [ "$INDEX_COUNT" -eq 3 ]; then
    ...
    echo "  OR manual: cat database/migrations/002_add_product_variations.sql | docker compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB""
    exit 0
else
    ...
    echo "     - Restore backup: cat backup.sql | docker compose exec -T postgres psql -U "$POSTGRES_USER" ucpready"
